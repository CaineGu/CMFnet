...
ViPCDataset_test = ViPCDataLoader('/root/XMFnet-10.2/dataset/test_list.txt', data_path=opt.dataroot, status= "test", category = "lamp")
...
gpus = [0,1]
model = nn.DataParallel(model,device_ids=[0,1],output_device=gpus[0])
model.to(device)

model_path = torch.load("/root/XMFnet-10.2/model_lamp_152.t7")
...
with torch.no_grad():
    ...
    for data in tqdm(test_loader):
        ...
        np.savetxt('/root/XMFnet-10.2/pt.txt',pt_pointcloud)    
        ...
        np.savetxt('/root/XMFnet-10.2/gt.txt',gt_pointcloud)    
        ...
        np.savetxt('/root/XMFnet-10.2/cp.txt',cp_pointcloud)    
