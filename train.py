...
def train_one_step(data, optimizer, network):
    ...
    partial_1 = farthest_point_sample(partial, 2048).permute(0, 2, 1)
    partial_2 = farthest_point_sample(partial, 1024).permute(0, 2, 1)
    partial_3 = farthest_point_sample(partial, 512).permute(0, 2, 1)
    partial = [partial_1, partial_2, partial_3]
    ...
    loss_test  = loss_cd_eval(complete, gt)
    optimizer.zero_grad()
    loss_total.backward()
    optimizer.step()

    return loss_total, loss_test
...
####################  训练  ###################
    ...
    for data in loop:
        ...
        loss, loss_test = train_one_step(data, optimizer, network=model)
        i += 1
            
        Loss += loss.item()
        Loss_test += loss_test.item()
        loop.set_description(f'Epoch [{epoch+1}/{opt.n_epochs}]')
        loop.set_postfix({'loss': '{0:1.8f}'.format(Loss / i), 'loss_test': '{0:1.8f}'.format(Loss_test / i)})
    ...
    with torch.no_grad():
        ...
        if Loss_eval < best_loss:
            ...
            best_epoch = epoch+1
            LossCD     = LOSSCD
            torch.save(model.state_dict(), f'/root/XMFnet-2080ti/checkpoints/ckpt_{opt.cat}.t7')
            # torch.save(model, f'./log/{MODEL}/{MODEL}_{VERSION}_{BATCH_SIZE}_{CLASS}_{FLAG}_{TIME_FLAG}/ckpt_chair_{epoch}.pt')
            print('model saved!')
    print('****************************')
    print("最佳训练轮次： ",best_epoch, ' ', '最佳损失为： ', best_loss, f'losscd 损失为： {LossCD}')
    print('****************************')
 
    outstr = 'best_epoch %d, best_loss: %.8f , best_losscd: %.8f' % (best_epoch, best_loss, LossCD)
    io.cprint(outstr)
